...
**Note on MVP mode (resolved 2026-09-10):** ROADMAP.md's Phase 1 entry carried `Mode: mvp`, but its Goal text originally failed `user-story.validate` (not in "As a ..., I want to ..., so that ...." form), which had blocked producing a User Flow Coverage table specifically (it would have been low-quality against a non-user-story goal) — it never blocked verification generally, which proceeded against the 5 ROADMAP Success Criteria (Step 2a's non-negotiable contract). The Goal has since been reworded into user-story form (see `resolved_human_verification` in frontmatter); `user-story.validate` now returns `valid: true`. The User Flow Coverage table below was produced in this same pass, per `references/verify-mvp-mode.md`'s required output format for MVP-mode phases.

## User Flow Coverage

User story: «As a visitor, I want to submit a location-tagged emergency report and see it alongside other nearby reports on a live map, so that I can report and track emergencies without needing to create an account.»

| Step | Expected | Evidence | Status |
|------|----------|----------|--------|
| Open the app | No signup or login is required — an anonymous session is issued automatically on the first request | `internal/api/router.go:70` mounts `deps.Session.Middleware(...)` ahead of every route; zero `/login`, `/register`, or `/signup` routes exist anywhere in `internal/api/router.go` or the client JS (grep-confirmed repo-wide); `TestSessionIssuance` (`internal/session/cookie_test.go:65`) confirms a first request with no cookie receives a fresh `Set-Cookie: pinalert_session=...`. Live re-verification this session: `curl -c jar -X POST /api/reports` with no prior cookie returned `201` with a `Set-Cookie` header, no auth challenge. | ✓ |
| Submit a location-tagged emergency report | Filling category, severity, description and a location, then submitting, persists the report | `internal/api/handlers/reports.go:145` `SubmitReport` handles `POST /api/reports`; `web/static/js/app.js:157` `submitReport` POSTs the modal's payload; `TestValidateSubmitInput` and `TestShelterCapacityValidation` (`internal/service/report_test.go`) pass fresh against all 9 categories, 3 severities, and shelter-capacity gating. Live re-verification: a `POST /api/reports` with `latitude`/`longitude`/`category`/`severity`/`description` returned `201` with the stored, echoed report. | ✓ |
| See it alongside other nearby reports on the map | After submitting, the new report and other pre-existing nearby reports both appear as pins on the Leaflet/OSM map — a bounding-box + Haversine query, not a full-table scan | `internal/api/handlers/reports.go:213` `NearbyReports` handles `GET /api/reports`; `TestNearbyReportsUsesIndex` confirms a real `EXPLAIN` shows an Index Scan, not a Seq Scan; `web/static/js/app.js:117` `fetchReports` calls the same endpoint; `web/static/js/map.js:191` renders each report as `L.marker([report.latitude, report.longitude], {...}).addTo(map)`. Live re-verification: `GET /api/reports?lat=...&lon=...&radius_km=5` returned the just-submitted report plus 4 pre-existing seeded reports, each with a real computed `distance_km`. Visual pin/map rendering additionally confirmed by the human tester (01-UAT.md Tests 1, 2, 5, 7, all passed). | ✓ |
| Outcome — report and track emergencies without needing to create an account | The entire flow above required no signup step at any point, and a submitted report stays visible/trackable in the feed and on the map until its expiry passes, checked live on read rather than by a background sweep | No signup/login route exists anywhere in the codebase (grep-confirmed, same evidence as Step 1); `TestExpiryReadTimePredicate` confirms a report is present before its TTL and absent immediately after, evaluated live on every read; grep-confirmed zero `time.Ticker`/cron-style background expiry job anywhere in the codebase. | ✓ |

**MVP outcome verdict:** ✓ VERIFIED — the user story's `[outcome]` clause ("I can report and track emergencies without needing to create an account") is observably true end-to-end against the current codebase and live re-verification; all 4 flow steps pass with no gaps.
